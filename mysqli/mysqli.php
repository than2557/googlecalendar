 ...
 * ****************************** */
  define( 'DBHOST', 'localhost' ); // set database host
  define( 'DBUSER', 'root' ); // set database user
  define( 'DBPASS', '' ); // set database password
  define( 'DBNAME', 'datameetingroomtest' ); // set database name
...
class DB {
    ...
    public function log_db_errors($error, $query) {
        ...
        if (defined('SEND_ERRORS_TO')) {
            $file = new SplFileObject(SEND_ERRORS_TO . "ERROR-DB-" . date('Y-m-d'), "a+");
            $written = $file->fwrite($message);
            $file = NULL;
        }
    }
    ...
    public function stat_boo($val) {
        if($val==1) return "Yes"; else return "No";
    }
      public function type_os($val) {
        if($val==1) return "Linux"; else 
            if($val==2) return "Windows";else 
                if($val==3) return "Android";else 
                    if($val==4) return "OSX"; else return "Unknown";
    }
    /**
